...
$step = 2; // isset( $_GET['step'] ) ? (int) $_GET['step'] : 0;
...
<p><?php _e( 'Please provide the following information. Do not worry, you can always change these settings later.' ); ?></p>
	...
	case 2:
		...
		$weblog_title         = 'titleHere'; // isset( $_POST['weblog_title'] ) ? trim( wp_unslash( $_POST['weblog_title'] ) ) : '';
		$user_name            = 'adminnameHere'; // isset( $_POST['user_name'] ) ? trim( wp_unslash( $_POST['user_name'] ) ) : '';
		$admin_password       = 'adminpassHere'; // isset( $_POST['admin_password'] ) ? wp_unslash( $_POST['admin_password'] ) : '';
		$admin_password_check = 'adminpassHere'; // isset( $_POST['admin_password2'] ) ? wp_unslash( $_POST['admin_password2'] ) : '';
		$admin_email          = 'emailHere'; // isset( $_POST['admin_email'] ) ? trim( wp_unslash( $_POST['admin_email'] ) ) : '';
		$public               = 1; // isset( $_POST['blog_public'] ) ? (int) $_POST['blog_public'] : 1;
		
		$error = false;
/* skip error check, we filled everything manually
		// Check email address.
		if ( empty( $user_name ) ) {
			// TODO: Poka-yoke.
